...
exports.tutorForm1 = async (req, res) => {
  //   let fundId_bank = "";
  //   let fundId_upi = "";
  const tutorForm1 = new Tutor_Form1({
    ...
    highestDegreeFile: req.body.highestDegreeFile,
    pan_name: req.body.panName,
    pan_number: req.body.panNumber,
    // pan_file:req.body.panFile,
  ...
  try {
    ...
    if (req.files) {
      ...
      //   tutorForm1.highestDegreeFile.push(file.path);
      // });

      // Path = Path.substring(0, Path.lastIndexOf(","));
      // tutorForm1.pan_file = Path;
          ...
          if (razorpayFund_bank.data) {
            ...
            if (razorpayBank_validation.data) {
              if (razorpayBank_validation.data.status == "completed") {
                if (
                  razorpayBank_validation.data.results.registered_name ==req.body.accName)
                 {
                  tutorForm1.fundId_bank = fa_id;
                  const newV = await tutorForm1.save();
                  res.status(200).json({success:true})
                } else {
                  res.send({ success: false, status: "notMatched", validationOne:razorpayBank_validation.data});
                  console.log(
                  ...
                  console.log(
                    {validationtWO:req.body.username}
                   );
                }
              }
              // else{
...
exports.email_check = async (req, res) => {
...
exports.find_tutorId = async (req, res) => {
