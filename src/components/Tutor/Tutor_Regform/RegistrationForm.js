...
const RegistrationForm = (props) => {
  ...
  const handleSkillChange = (e) => {
    // const newSkill = subjects.filter((i) => i === e.target.value);
    // const newSkill = skill.filter((j) => j === i);
    // if (newSkill.length === 0 && skill.length < 20) {
    //   setSkill([...skill, i]);
    //   props.setData({
    ...
    // var sub=subjects.filter(name => name.toLowerCase().includes(skillChange))
    // if(sub!==null){
    //   setSubjects1(sub)
    // }
    // else{
  ...
  const handleNext = async (e) => {
    ...
    if (
      ...
      history.replace("/TutorSignIn/PAN_Card_Check");
    } else {
